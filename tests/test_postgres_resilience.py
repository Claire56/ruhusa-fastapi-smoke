...
# Module-scoped client keeps the PostgreSQL pool open for both destructive
# tests.  If each test created its own TestClient context the shutdown handler
# would close runtime.pool after the first test, causing the second to fail
# with PoolClosed.  raise_server_exceptions=False lets us assert on 5xx
# responses during the outage without the test client raising an exception.
...
def test_database_outage_blocks_protected_side_effect_and_pool_recovers(client):
...
def test_postgres_container_restart_preserves_execution_and_audit_state(client):
